...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        let deserialized = fs::read_to_string(&self.file_path)?;
        let db_string: DBState = serde_json::from_str(&deserialized)?;
        Ok(db_string)
    }

    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let serialized_dbstate = serde_json::to_string(&db_state)?;
        fs::write(&self.file_path, serialized_dbstate)?;
        Ok(())
    }
}
